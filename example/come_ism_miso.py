...
N = int(2**(np.ceil(np.log2(max_delay))))
# todo: use the next largest devisor of `fs`
N = 1600
...
for m in range(M):
    ...
    for i, source_pos in enumerate(xs):
        ...
        s += captured_signal(waveform * scale[i], shift, np.roll(p, m*N))
